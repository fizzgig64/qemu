    ...
    switch (delivery_mode) {
        case APIC_DM_LOWPRI:
            /* XXX: search for focus processor, arbitration */
            {
                ...
                if (d >= 0) {
                    ...
                    if (apic_iter) {
                        apic_set_irq_remote(apic_iter, vector_num, trigger_mode); /* GVM add _remote */
                    }
                }
            }
            return;
        ...
        case APIC_DM_SMI:
            foreach_apic(apic_iter, deliver_bitmask,
                cpu_interrupt_remote(CPU(apic_iter->cpu), CPU_INTERRUPT_SMI) /* GVM add _remote */
            );
            return;

        case APIC_DM_NMI:
            foreach_apic(apic_iter, deliver_bitmask,
                cpu_interrupt_remote(CPU(apic_iter->cpu), CPU_INTERRUPT_NMI) /* GVM add _remote */
            );
            return;
        ...
        case APIC_DM_INIT:
            ...
            foreach_apic(apic_iter, deliver_bitmask,
                cpu_interrupt_remote(CPU(apic_iter->cpu), CPU_INTERRUPT_INIT) /* GVM add _remote */
            );
            return;
...
void apic_startup(CPUState *cpu, int vector_num) /* GVM add: remove static */
{

    APICCommonState *s = APIC(X86_CPU(cpu)->apic_state); /* GVM add */
    s->sipi_vector = vector_num;

    /* GVM add begin: had called cpu_interrupt(CPU(s->cpu), CPU_INTERRUPT_SIPI); */
    /*
                         ...
                         uint8_t trigger_mode)
{
    ...
    switch (delivery_mode) {
        case APIC_DM_INIT:
            {
                ...
                if (level == 0 && trig_mode == 1) {
                    foreach_apic(apic_iter, deliver_bitmask,
                                 apic_init_level_deassert_remote(apic_iter)); /* GVM add: used to assign */
                    return;
                }
            }
            break;
        ...
        case APIC_DM_SIPI:
            foreach_apic(apic_iter, deliver_bitmask,
                         apic_startup_remote(CPU(apic_iter->cpu), vector_num) ); /* GVM add: used to assign */
            return;
    }
...
static void apic_send_msi(MSIMessage *msi)
{
    ...
    DeviceState *dev = cpu_get_current_apic(); /* GVM add */
...
void apic_mem_writel(void *opaque, hwaddr addr, uint32_t val) /* GVM add: remove static */
{
    DeviceState *dev;
    APICCommonState *s;
    int index = (addr >> 4) & 0xff;
